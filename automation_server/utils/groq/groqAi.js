...
class GroqAi {
  ...
  static async testApi() {
    console.log("\n[TEST API] Memulai testing API Keys...\n");
    ...
    for (let i = 0; i < apiKeys.length; i++) {
      ...
      try {
        ...
        console.log(
          `[Key ${i + 1}/${apiKeys.length}] ✅ Status: 200 OK | Latency: ${elapsed}ms`,
        );
      } catch (err) {
        console.log(
          `[Key ${i + 1}/${apiKeys.length}] ❌ Status: ${err.status || "ERROR"} | Msg: ${err.message}`,
        );
      }
    }
    console.log("\n[TEST API] Selesai.\n");
  ...
  static async generateImage(image) {
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (err.status === 429 || err?.error?.code === "rate_limit_exceeded") {
          console.log(
            `[Rate Limit] API Key at index ${this.currentKeyIndex} hit rate limit. Rotating to next key...`,
          );
    ...
    throw new Error("All API keys hit rate limits or failed. Backing off...");
  }
}
