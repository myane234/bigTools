  ...
  const waitForImages = async (targetCount, timeoutMs = 60000) => {
    ...
    const scanImages = async () => {
      ...
      for (const img of imgLocators) {
        ...
        if (!imageInfo?.src || !imageInfo.complete || !imageInfo.width || !imageInfo.height) {
          continue;
        }
    ...
    const scrollAndScan = async () => {
      // Langkah lebih kecil memastikan gambar lazy-loaded di setiap bagian canvas ikut muncul.
      ...
      // Sapu balik dari bawah ke atas agar lazy-loaded image yang terlewat ikut diproses.
